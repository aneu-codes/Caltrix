class MatrixCalculator {
	...
	adjunctMatrix() {
		...
		for (i = 0; i < this.AyDimension; i++) {
			...
			for (var j = 0; j < this.AxDimension; j++) {
				...
				if (this.AxDimension == 2) {
					...
					if (i == 0) {
						if (j == 0) {
							step1[i][j] = `C[${i + 1}][${j + 1}] = (-1^(${i + 1}+${j + 1})) * M${i + 1}${j + 1} = ${(-1) ** (i + 1 + j + 1)} * ${this.matrixA[1][1]} = ${this.matrixA[1][1]}`;
							stepi[i][j] = `${(-1) ** (i + 1 + j + 1)} * ${this.matrixA[1][1]}`;
						} if (j == 1) {
							step1[i][j] = `C[${i + 1}][${j + 1}] = (-1^(${i + 1}+${j + 1})) * M${i + 1}${j + 1} = ${(-1) ** (i + 1 + j + 1)} * ${-this.matrixA[1][0]} = ${-this.matrixA[1][0]}`;
							stepi[i][j] = `${(-1) ** (i + 1 + j + 1)} * ${-this.matrixA[1][0]}`;
						}
					} else {
						step1[i][j] = `C[${i + 1}][${j + 1}] = (-1^(${i + 1}+${j + 1})) * M${i + 1}${j + 1} = ${(-1) ** (i + 1 + j + 1)} * ${adjacent[i][j]} = ${adjacent[i][j]}`;
						stepi[i][j] = `${(-1) ** (i + 1 + j + 1)} * ${adjacent[i][j]}`;
					}
				}

				//Calculando cofactor matrix
				if (this.AxDimension == 3) {
					...
					step1[i][j] = `C[${i + 1}][${j + 1}] = [-1^(${i + 1}+${j + 1})] * det(M${i + 1}${j + 1}) = ${(-1) ** (i + 1 + j + 1)} * [(${aux[0][0]} * ${aux[1][1]}) - (${aux[0][1]} * ${aux[1][0]})] = ${adjacent[i][j]} `;
		...
		for (var i = 0; i < this.AxDimension; i++) {
			...
			for (var j = 0; j < this.AyDimension; j++) {
				...
				step2[i][j] = `C[${j + 1}][${i + 1}] = ${adjacent[j][i]}   ->   C^t[${j + 1}][${i + 1}] = adj(A)[${i + 1}][${j + 1}] = ${adjacent[j][i]}`;
			}
		}
	...
	scalarXMatrix() {
		...
		for (i = 0; i < this.AyDimension; i++) {
			...
			for (var j = 0; j < this.AxDimension; j++) {
				...
				steps[i][j] = `A[${i + 1}][${j + 1}] * ${scalar} = ${this.matrixA[i][j]} * ${scalar} = ${result[i][j]}`;
				step1[i][j] = `${this.matrixA[i][j]} * ${scalar}`;
			}
		}
	...
	subtractMatrix() {
		...
		if (this.AxDimension != this.AyDimension) {
			this.printOnConsole("Recuerda: Solo se pueden sustraer matrices con igual dimensión.");
			return;
		}
		var result = [];
		for (var i = 0; i < 3; i++)
		...
		for (i = 0; i < this.AyDimension; i++) {
			...
			for (var j = 0; j < this.AxDimension; j++) {
				...
				steps[i][j] = `A[${i + 1}][${j + 1}] - B[${i + 1}][${j + 1}] = ${this.matrixA[i][j]} - ${this.matrixB[i][j]} = ${result[i][j]}`
				step1[i][j] = `${this.matrixA[i][j]} - ${this.matrixB[i][j]}`
			}
		}
	...
	multiplyMatrix() {
		...
		for (i = 0; i < rowsRes; i++) {
			...
			for (j = 0; j < columnsRes; j++) {
				...
				steps[i][j] = `A[${i + 1}][${0 + 1}] * B[${0 + 1}][${j + 1}] + A[${i + 1}][${1 + 1}] * B[${1 + 1}][${j + 1}] + A[${i + 1}][${2 + 1}] * B[${2 + 1}][${j + 1}] = ${this.matrixA[i][0]} * ${this.matrixB[0][j]} + ${this.matrixA[i][1]} * ${this.matrixB[1][j]} + ${this.matrixA[i][2]} * ${this.matrixB[2][j]} = ${result[i][j]}`;
	...
	calculateDeterminant() {
		...
		if (this.AxDimension == 3) {
			...
			steps.push(`d1 = A[${0 + 1}][${0 + 1}] * A[${1 + 1}][${1 + 1}] * A[${2 + 1}][${2 + 1}] = ${this.matrixA[0][0]} * ${this.matrixA[1][1]} * ${this.matrixA[2][2]} = ${d1}`);
			steps.push(`d2 = A[${0 + 1}][${1 + 1}] * A[${1 + 1}][${2 + 1}] * A[${2 + 1}][${0 + 1}] = ${this.matrixA[0][1]} * ${this.matrixA[1][2]} * ${this.matrixA[2][0]} = ${d2}`);
			steps.push(`d3 = A[${0 + 1}][${2 + 1}] * A[${1 + 1}][${0 + 1}] * A[${2 + 1}][${1 + 1}] = ${this.matrixA[0][2]} * ${this.matrixA[1][0]} * ${this.matrixA[2][1]} = ${d3}`);
			steps.push(`d4 = A[${0 + 1}][${2 + 1}] * A[${1 + 1}][${1 + 1}] * A[${2 + 1}][${0 + 1}] = ${this.matrixA[0][2]} * ${this.matrixA[1][1]} * ${this.matrixA[2][0]} = ${d4}`);
			steps.push(`d5 = A[${0 + 1}][${0 + 1}] * A[${1 + 1}][${2 + 1}] * A[${2 + 1}][${1 + 1}] = ${this.matrixA[0][0]} * ${this.matrixA[1][2]} * ${this.matrixA[2][1]} = ${d5}`);
			steps.push(`d6 = A[${0 + 1}][${1 + 1}] * A[${1 + 1}][${0 + 1}] * A[${2 + 1}][${2 + 1}] = ${this.matrixA[0][1]} * ${this.matrixA[1][0]} * ${this.matrixA[2][2]} = ${d6}`);
			steps.push(`det(A) = d1 + d2 + d3 - d4 - d5 - d6 = ${d1} + ${d2} + ${d3} - ${d4} - ${d5} - ${d6} = ${determinant}`);
		}
